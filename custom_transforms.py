...
# Individual Images
#######################
class COVIDNetProcessing(object):
    def __init__(self,percent):
        self.percent = percent
    def crop_top(self,img):
        offset = int(img.shape[-2] * self.percent)
        if img.ndim == 3:
            output = img[:,offset:,:]
        elif img.ndim == 2:
            output = img[offset:,:]
    ...
    def central_crop(self,img):
        size = min(img.shape[-2], img.shape[-1])
        offset_h = int((img.shape[-2] - size) / 2)
        offset_w = int((img.shape[-1] - size) / 2)
        if img.ndim == 3:
            output = img[:, offset_h:offset_h + size, offset_w:offset_w + size]
        elif img.ndim == 2:
            output = img[offset_h:offset_h + size, offset_w:offset_w + size]
        return output
    def __call__(self, img):
        # Input is [CxHxW] image
        # Cut top few % of image, then center-crop, then resize
        if self.percent !=0:
...
class HistogramEqualisation(object):
    ...
    def __call__(self, sample):
        ...
        return sample
################
# Both TORCH TENSOR and PIL IMAGE
################
class Resize(object):
    """
